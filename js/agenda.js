// agenda.js — núcleo: instrumento, formulário, gravar, editar, filtrar, P&L, processar
    ...
    function gerarCelulaPnL(op) {
        ...
        const sinal = pnl >= 0 ? '+' : '-';
        return `<td style="color:${cor};font-weight:700;">${sinal}$${Math.abs(pnl).toFixed(2)}</td>`;
    }
    ...
    function processarAgenda() {
        ...
        agendaOrdenada.forEach(op => {
            ...
            if (passaFiltroAtivo && passaFiltroTipo) {
                ...
                linhasHTML.push(`<tr>
                    ...
                    <td class="${corValor}">${sinal}$${Math.abs(op.valorEfeito).toFixed(2)}</td>
        ...
        if (filtro === 'TODOS') {
            ...
            document.getElementById('resumoTotalAplicado').innerText = `$${Math.max(0,din).toFixed(2)}`;
