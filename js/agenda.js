// agenda.js — núcleo: instrumento, formulário, gravar, editar, filtrar, P&L, processar
    ...
    // ===== OPÇÕES VENDIDAS EM ABERTO — derivadas automaticamente do Histórico =====
    // Substitui a antiga lista manual POSICOES_CONSOLIDADAS.opcoesVendidas (estado.js).
    // Considera "aberta" uma venda de opção que: ainda não tem prémio de fechamento
    // registado (não foi recomprada/rolada) E ainda não passou da data de exercício.
    // Roda de novo sempre que chamada — nunca fica desatualizada quando um item é
    // adicionado, editado (ex: preencher o prémio de fechamento) ou apagado.
    function calcularOpcoesVendidas() {
        const hoje = new Date(); hoje.setHours(0,0,0,0);
        return agenda
            .filter(op =>
                op.instrumento === 'OPCAO' &&
                op.direcao === 'VENDA' &&
                (op.premioFechamento === null || op.premioFechamento === undefined) &&
                op.dataExercicio &&
                new Date(op.dataExercicio + 'T00:00:00') >= hoje
            )
            .map(op => ({
                ticker: op.ticker,
                strike: op.preco,
                tipo: op.tipoOpcao === 'CALL' ? 'C' : 'P',
                vencimento: op.dataExercicio,
                contratos: op.qtdOriginal / 100,
                moeda: op.moeda || 'USD'
            }));
    }

    // ===== PRÉMIO TOTAL RECEBIDO — TODO o histórico de vendas de CALL/PUT =====
    // Diferente de calcularOpcoesVendidas() (só posições ainda abertas, usada pro gráfico
    // e pro "Valor Retido"), esta soma TODA venda de opção já registada, aberta, fechada
    // ou expirada — é "quanto já foi ganho em prémio", não "quanto está em risco agora".
    // Cada operação já grava o valor em módulo (Math.abs), convertido pra USD conforme a
    // moeda da própria operação (op.moeda), pra não misturar BRL/EUR com USD na soma.
    function calcularPremioTotalOpcoes() {
        return agenda
            .filter(op => op.instrumento === 'OPCAO' && op.direcao === 'VENDA')
            .reduce((total, op) => total + converterParaUSD(Math.abs(op.valorEfeito), op.moeda || 'USD'), 0);
    }
