// totais.js — cartões de categoria, cash Tastytrade e totais consolidados
    ...
    async function calcularTotaisConsolidados() {
        ...
        const elPremioHistorico = document.getElementById('totalPremioHistorico');
        ...
        // Opções vendidas agora vêm de calcularOpcoesVendidas() (agenda.js) — derivadas
        // do Histórico, não mais da lista manual POSICOES_CONSOLIDADAS (removida).
        const acoesAtuais = calcularAcoesDetidas();
        const opcoesAtuais = calcularOpcoesVendidas();
        const [resultadosAcoes, resultadosOpcoes] = await Promise.all([
            Promise.all(acoesAtuais.map(pos => buscarPrecoYahoo(pos.ticker))),
            Promise.all(opcoesAtuais.map(pos => buscarPrecoOpcaoYahoo(pos)))
        ]);
        ...
        // Prémio Total Recebido (Histórico): TODAS as vendas de CALL/PUT já registadas,
        // abertas ou já fechadas/expiradas — não é a mesma conta de "Valor Retido" acima
        // (que é só exposição das posições ainda em aberto, ao preço de recompra atual).
        if (elPremioHistorico) {
            const premioHistorico = calcularPremioTotalOpcoes();
            elPremioHistorico.innerText = formatarMoeda(premioHistorico, 'USD');
        }
