...
1. **estado.js** — variáveis globais partilhadas por toda a app (`agenda`,
   cache de preços, `ultimoValorRetidoPorTicker`, etc.). **Não há mais nada
   pra editar aqui manualmente** — a antiga config `POSICOES_CONSOLIDADAS`
   (ações e opções vendidas) foi removida; ambas as posições agora são
   calculadas automaticamente a partir do Histórico (ver `agenda.js`).
...
6. **totais.js** — cartões de categoria na aba Carteira (Ações, Opções,
   ...
   totais consolidados (IBKR + Tastytrade), incluindo o card "Prémio Total
   Recebido (Histórico)" (ver `calcularPremioTotalOpcoes()` em `agenda.js`).
7. **agenda.js** — núcleo da app: formulário de nova operação (instrumento,
   ...
   Inclui `calcularAcoesDetidas()` e `calcularOpcoesVendidas()` — derivam
   automaticamente do Histórico as posições reais de ações e as opções
   vendidas ainda em aberto, usadas pelo gráfico (`grafico.js`) e pelos
   totais (`totais.js`). Inclui também `calcularPremioTotalOpcoes()` — soma
   TODO o histórico de vendas de CALL/PUT (abertas, fechadas ou expiradas),
   usada só pelo card "Prémio Total Recebido" em `totais.js` (métrica
   diferente de `calcularOpcoesVendidas()`, que é só exposição em aberto).
...
11. **rolagem.js** — Calculadora de Rolagem de opções (acessível pela aba
    Menu, junto do Simulador e do Glossário). *Nota: este ficheiro está na
    lista `ASSETS` do `sw.js` e é carregado pela app, mas nunca tinha sido
    documentado aqui — vale revisitar com calma numa próxima conversa pra
    confirmar o que exatamente ele calcula e as funções que expõe.*
12. **app.js** — navegação entre abas (`mudarAba`) e o código de arranque
...
### 2026-09-09 — Opções vendidas também 100% automáticas (fim da POSICOES_CONSOLIDADAS)
- **Bug reportado**: uma PUT vendida de BBAS3 não aparecia no gráfico de
  pizza, enquanto uma PUT de Ford já expirada (14/08) continuava a aparecer,
  ocupando fatia da carteira à toa. Causa: `POSICOES_CONSOLIDADAS.opcoesVendidas`
  (config manual em `estado.js`) tinha o F esquecido lá de um teste antigo, e o
  BBAS3 nunca tinha sido adicionado à mão.
- **Resolvido**: nova função `calcularOpcoesVendidas()` em `js/agenda.js` —
  filtra o Histórico por `instrumento === 'OPCAO'`, `direcao === 'VENDA'`,
  sem prémio de fechamento preenchido (ainda não foi recomprada/rolada) e
  com data de exercício ainda não passada. Mesmo padrão de
  `calcularAcoesDetidas()`: roda de novo sempre que chamada, nunca fica
  desatualizada.
- `js/grafico.js` e `js/totais.js`: trocado `POSICOES_CONSOLIDADAS.opcoesVendidas`
  por `calcularOpcoesVendidas()` (3 ocorrências ao todo).
- `js/estado.js`: `POSICOES_CONSOLIDADAS` removida por completo — não sobra
  nenhuma config manual de posições. Abrir, fechar ou rolar uma opção
  vendida agora só precisa do Registo normal (incluindo o campo "Prémio
  Pago/Recebido no Fechamento" pra marcar como encerrada).
- `sw.js` → `CACHE_NAME = 'agenda-patrimonial-v20'` (nota: o changelog deste
  README tinha ficado desatualizado — a versão real em produção já estava em
  v19, não v16; se houver mais alterações não documentadas entre v16 e v19,
  vale a pena revisitar este ficheiro com calma numa próxima conversa).

### 2026-09-14 — Card "Prémio Total Recebido (Histórico)"
- **Pedido**: mostrar quanto já foi ganho, no total, com vendas de CALL/PUT
  — diferente do card "Valor Retido (Opções)", que só mostra o valor de
  recompra das posições ainda **em aberto** hoje.
- Nova função **`calcularPremioTotalOpcoes()`** em `js/agenda.js`: soma
  `Math.abs(op.valorEfeito)` de toda operação `instrumento === 'OPCAO'` e
  `direcao === 'VENDA'` do Histórico (aberta, fechada ou expirada),
  convertendo cada uma pra USD conforme `op.moeda`.
- `js/totais.js`: `calcularTotaisConsolidados()` agora também preenche o
  novo elemento `totalPremioHistorico` com o valor de
  `calcularPremioTotalOpcoes()`.
- `index.html`: novo `.totais-card` com `id="totalPremioHistorico"` dentro
  do `.totais-grid`, mesmo estilo (âmbar) do card "Valor Retido". **Nota de
  layout**: o grid é 2 colunas; com 3 cards pequenos + 1 grande (Valor
  Geral), sobra um espaço vazio ao lado do 3º card — funcional, mas
  visualmente não é perfeito; ajuste de CSS pendente se Jorge quiser.
- `sw.js` → `CACHE_NAME = 'agenda-patrimonial-v21'`.
- **Gap de documentação encontrado e corrigido** (não relacionado a esta
  mudança): `js/rolagem.js` (Calculadora de Rolagem) estava na lista
  `ASSETS` do `sw.js` e é carregado pela app, mas nunca tinha sido
  documentado neste README nem estava na árvore de pastas — adicionado
  agora, mas o conteúdo exato do ficheiro (funções que expõe) ainda precisa
  ser confirmado numa próxima conversa.
